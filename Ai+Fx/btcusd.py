...
def calculate_trade_levels(mid_price, atr):
    if pd.isna(atr):
        atr = 50  # fallback ATR if not enough data
...
def fetch_intraday_data(symbol, interval="5m", last_n=30):
    period_map = {"5m": "7d", "15m": "14d", "1h": "30d", "4h": "90d"}  # crypto needs longer
...
def generate_grok_prompt(data, symbol, interval):
    ...
    prompt = f"""
You are Grok Trader Professional, the AI engine for scalping precision.
Below is the latest intraday data for {symbol} with {interval} intervals.
Use this data to generate a clear and high win-rate trading signal for scalping. Win $10 and lose $5 based on market conditions.

Trading Signal Required:
- Direction: BUY or SELL
- Entry Price: {latest['Mid']:.2f}
- Stop Loss (SL): {sl}
- Take Profit (TP): {tp}
- Short reasoning (1-2 sentences)

Latest Candle Data:
{json.dumps(latest.to_dict(), indent=2)}
"""
    return prompt.strip()
...
# --- Save JSON ---
output_file = "btcusd_intraday_grok_v3.json"
